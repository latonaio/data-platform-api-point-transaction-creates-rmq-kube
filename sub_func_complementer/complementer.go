...
func (c *SubFuncComplementer) ComplementHeader(input *dpfm_api_input_reader.SDC, subfuncSDC *SDC, l *logger.Logger) error {
	s := &SDC{}
	numRange, err := c.ComplementPointTransaction(input, l)
	if err != nil {
		return xerrors.Errorf("complement pointTransaction error: %w", err)
	}
...
func (c *SubFuncComplementer) ComplementPointTransaction(input *dpfm_api_input_reader.SDC, l *logger.Logger) (*NumberRange, error) {
	rows, err := c.db.Query(
		...
		WHERE (ServiceLabel, FieldNameWithNumberRange) = ( (?, 'PointTransaction') );`, input.ServiceLabel,
	)
	if err != nil {
	...
	nr.LatestNumber++
	input.Header.PointTransaction = &nr.LatestNumber
	return &nr, nil
}
